# GLM.md — Agent Handoff Guide (GLM 5.2)

> **Audience:** GLM 5.2 AI agents picking up work on the Cybershare Lead Scraper codebase.
...
## 2. GLM 5.2 — Sandbox-Specific Notes (READ FIRST)

These are the **critical** things to know before running any command in the sandbox. The sandbox differs from a normal dev machine in important ways.

### ⚠️ `.env` gets reset to SQLite by the sandbox
- The sandbox sometimes **overwrites `.env`** with the SQLite default from `.env.example` on session restart.
- **Before ANY DB command**, verify the env file:
  ```bash
  cat .env | grep DATABASE_URL
  ```
- Expected: `DATABASE_URL=postgresql://neondb_owner:NEON_PASSWORD_HERE@ep-round-frost-ad1cks0l...neon.tech/neondb?sslmode=require`
- If you see `DATABASE_URL=file:...sqlite` or similar, restore it:
  ```bash
  cat > .env <<'EOF'
...
### ⚠️ Dev server may die in the sandbox — use `unset DATABASE_URL`
- Long-running `bun run dev` processes can hang or die in the sandbox.
- If `bun` commands hang or crash with weird Prisma errors, **unset DATABASE_URL** first to force a fresh resolve:
  ```bash
  unset DATABASE_URL
  # Then re-export it inline for the specific command:
  ...
  ```
- If `bun run dev` still dies, try `bun --bun run dev` or check `dev.log` for the error.

### GLM 5.2 strengths on this stack
- GLM 5.2 handles **TypeScript + Next.js 16 App Router** well — use that confidence.
- Use the **`fullstack-dev` skill** (Next.js 16 + TypeScript + Tailwind 4 + shadcn/ui + Prisma) for new UI views or API routes. It generates code matching this repo's patterns.
- For AI feature work, use the **`LLM` skill** (z-ai-web-dev-sdk) — but note this project uses Mistral directly via `fetch`, not the SDK. Don't replace it unless explicitly asked.
- For UI testing, use the **`agent-browser` skill** (Playwright-based, faster than manual checks).
...
# 3. VERIFY .env (sandbox may have reset it — see §2)
cat .env | grep DATABASE_URL
# Must show: postgresql://neondb_owner:...neon.tech/neondb?sslmode=require
...
### Mistral AI
- **API key:** `MISTRAL_API_KEY_HERE`
- Model: `mistral-small-latest`
- Rate limit: 1 req/sec (enforced in-process via 1100ms delay)
- Endpoint: `https://api.mistral.ai/v1/chat/completions`

### Webshare Proxies
- Credentials: username `WEBSHARE_USER`, password `WEBSHARE_PASS`
- 10 free residential proxies, 1 GB/mo bandwidth
- Format: `http://WEBSHARE_USER:WEBSHARE_PASS@<host>:<port>`
- ⚠️ May hit 1 GB limit — `proxy-test.ts` detects HTTP 402 ("Bandwidth limit reached")
- Webshare login: `manny@cybershare.tech` / `PASSWORD_HERE`
...
## 6. Architecture (read this before touching the scraper)
...
Phase 2 (enrich)   30-70%  → Open each placeUrl for phone/website/address.
                              Upsert lead with enriched data. Concurrency=3.
...
- **GitHub repo:** `dustindog101/leadscraper`
- **To start watcher:**
  ```bash
...
- **`CLAUDE.md`** — same project, Claude-targeted (mostly identical content)
